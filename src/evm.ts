import { DeviceActionStatus, type ExecuteDeviceActionReturnType } from "@ledgerhq/device-management-kit";
...
export type DeviceActionRun<Output> = ExecuteDeviceActionReturnType<Output, unknown, unknown>;
 ...
 */
export function runDeviceAction<Output>(run: DeviceActionRun<Output>): Promise<Output> {
  return new Promise<Output>((resolve, reject) => {
    run.observable.subscribe({
      next: (state) => {
        switch (state.status) {
          ...
          case DeviceActionStatus.Stopped:
            reject(new DeviceActionError("Device action was cancelled before it completed", undefined));
            break;
          default:
      ...
      complete: () =>
        reject(new DeviceActionError("Device action ended without reporting a result", undefined)),
    });
  });
}
...
export function toDeviceActionError(error: unknown): DeviceActionError {
  ...
  const errorCode = typeof fields.errorCode === "string" ? fields.errorCode : undefined;
  const message =
    typeof fields.message === "string" ? fields.message : error instanceof Error ? error.message : undefined;
  const detail = [tag, errorCode, message]
    .filter((part): part is string => part !== undefined && part.length > 0)
    .join(": ");
  const statusCode =
    errorCode !== undefined && /^[0-9a-f]{4}$/i.test(errorCode) ? parseInt(errorCode, 16) : undefined;
...
/** Shapes a kit signature the way `hw-app-eth`'s `signPersonalMessage` returned it. */
export function toEvmMessageSignature(signature: Signature): {
  r: string;
  s: string;
  v: number;
} {
